...
(function qhtmlTypesModule(globalScope) {
  ...
  function qhtmlSplitCallArguments(source) {
    const args = [];
    let current = "";
    let depth = 0;
    let quote = "";
  ...
  function qhtmlResolveExpressionValue(expression, contextNode, resolving = new Set(), depth = 0) {
    ...
    const callMatch = /^([A-Za-z_$][A-Za-z0-9_$]*(?:\.[A-Za-z_$][A-Za-z0-9_$]*)*)\s*\(([\s\S]*)\)$/.exec(source);
    if (callMatch) {
      const callable = qhtmlResolveExpressionValue(callMatch[1], contextNode, resolving, depth + 1);
      if (typeof callable === "function") {
        const args = qhtmlSplitCallArguments(callMatch[2]).map(arg => qhtmlResolvePropertyValue(arg, contextNode, resolving, depth + 1));
        resolving.delete(source);
    ...
    if (parts[0] === "this") {
      ...
      if (!current) {
        const componentContext = qhtmlComponentThisFor(contextNode);
        if (componentContext && componentContext !== contextNode && typeof componentContext.qhtmlResolve === "function") {
          current = componentContext.qhtmlResolve(parts[0]);
        }
      }
    ...
    function unwrapExpressionValue(value) {
      ...
      if (value instanceof QHTMLVar) {
        return qhtmlVarValueForContext(value, value.parent() || contextNode);
      }
  ...
  function qhtmlScriptComponentProxy(componentNode) {
    return new Proxy(componentNode, {
      get(target, property) {
        ...
        const resolved = target.qhtmlResolve(property);
        if (resolved instanceof QHTMLProperty || resolved instanceof QHTMLPropertyAssignment) {
          return qhtmlResolvePropertyValue(resolved.value(), resolved.parent() || target);
        }
        if (resolved instanceof QHTMLVar) {
          return qhtmlVarValueForContext(resolved, resolved.parent() || target);
        }
        if (resolved instanceof QHTMLContextPropertyPointer) {
          return resolved.resolveTarget(target.rootNode());
        }
        if (resolved instanceof QHTMLKeyword) {
          return qhtmlScalarValue(resolved.value());
        }
        return resolved;
      },
      set(target, property, value) {
        ...
        qhtmlSetScopedScriptValue(null, target, property, value);
        return true;
      }
    });
  }

  function qhtmlSetScopedScriptValue(contextNode, componentNode, property, value) {
    const name = String(property || "");
    const context = contextNode && contextNode.qhtmlContext ? contextNode.qhtmlContext : null;
    if (context && context.hasContextProperty(name)) {
      context.setContextProperty(name, value);
      return true;
    }
    const resolved = contextNode && typeof contextNode.resolve === "function" ? contextNode.resolve(name) : null;
    if (resolved instanceof QHTMLVar) {
      resolved.setValue(valueToSource(value));
      return true;
    }
    const componentResolved = componentNode && typeof componentNode.qhtmlResolve === "function"
      ? componentNode.qhtmlResolve(name)
      : null;
    if (componentResolved instanceof QHTMLVar) {
      componentResolved.setValue(valueToSource(value));
      return true;
    }
    if (componentNode) {
      componentNode.setPropertyValue(name, String(value == null ? "" : value));
      return true;
    }
    return false;
  }

  function qhtmlInlineScriptScopeProxy(contextNode, componentNode, componentProxy) {
    const context = contextNode && contextNode.qhtmlContext ? contextNode.qhtmlContext : null;
    return new Proxy({}, {
      has(target, property) {
        if (typeof property === "symbol") {
        ...
        return context ? context.visibleKeys().includes(property) : false;
      },
      get(target, property) {
        ...
        return qhtmlResolveExpressionValue(property, contextNode);
      },
      set(target, property, value) {
        if (typeof property === "symbol") {
          target[property] = value;
        ...
        qhtmlSetScopedScriptValue(contextNode, componentNode, property, value);
        return true;
      }
    });
  }

  function qhtmlExecuteInlineScriptAction(actionNode, contextNode, forNode) {
    const runner = globalScope && globalScope.__qhtmlInlineScriptActionRunner;
    if (typeof runner === "function") {
      return runner(actionNode, contextNode, forNode);
    }
    const body = typeof actionNode.body === "function" ? actionNode.body() : "";
    const componentNode = qhtmlComponentThisFor(contextNode);
    const componentProxy = qhtmlScriptComponentProxy(componentNode);
    const scopeProxy = qhtmlInlineScriptScopeProxy(contextNode, componentNode, componentProxy);
    return Function("scope", "with (scope) {\n" + qhtmlScriptBody(body) + "\n}").call(componentProxy, scopeProxy);
  }

  function qhtmlVarBodyIsFunction(varNode) {
    return Boolean(varNode && typeof varNode.body === "function" && trim(varNode.body()) !== "");
  }

  function qhtmlVarFunctionBody(varNode) {
    const body = trim(varNode && typeof varNode.body === "function" ? varNode.body() : "");
  ...
  function qhtmlVarFunctionForContext(varNode, contextNode) {
    const componentNode = qhtmlComponentThisFor(contextNode);
    const componentProxy = qhtmlScriptComponentProxy(componentNode);
    const scopeProxy = qhtmlInlineScriptScopeProxy(contextNode, componentNode, componentProxy);
    const body = qhtmlVarFunctionBody(varNode);
    return function qhtmlVarFunction(...args) {
      return Function("scope", "args", "with (scope) {\n" + qhtmlScriptBody(body) + "\n}").call(componentProxy, scopeProxy, args);
    };
  }

  function qhtmlVarValueForContext(varNode, contextNode) {
    if (qhtmlVarBodyIsFunction(varNode)) {
      return qhtmlVarFunctionForContext(varNode, contextNode);
    }
    const value = typeof varNode.value === "function" ? varNode.value() : "";
    return qhtmlResolvePropertyValue(value, contextNode || (varNode && varNode.parent ? varNode.parent() : null));
  }
  ...
  class QHTMLNode extends QHTMLReference {
    ...
    referenceRevision() { return this._qhtmlReferenceRevision; }
    referenceRevisionJs() { return this.referenceRevision(); }
    invalidateReferenceCache() {
      qhtmlGlobalReferenceRevision += 1;
      let current = this;
      while (current) {
        current._qhtmlReferenceRevision = qhtmlGlobalReferenceRevision;
        current = typeof current.parent === "function" ? current.parent() : null;
      }
    ...
    static nodeFromJsonObject(object, ownerScope = null) {
      ...
      switch (type) {
        ...
        case "QHTMLClass":
          ...
          break;
        case "QHTMLVar":
          node = new QHTMLVar(name, Object.assign({}, attributes, { value: firstString(object, ["value"], attributes.value || "") }), readBody(object));
          break;
        case "QHTMLScript":
  ...
  class QHTMLDomElement extends QHTMLDomNode {
    ...
    assignmentAttributesForContext(contextNode = this) {
      ...
      for (const child of this.children()) {
        ...
        out.push(" " + name + "=\"" + qhtmlEscapeAttribute(qhtmlResolvePropertyValue(child.value(), contextNode)) + "\"");
        emitted.add(lower);
      }
      return out.join("");
    }
  ...
  function materializeIterationValue(node, variableName, item) {
    ...
    if (node instanceof QHTMLVar &&
        typeof node.value === "function" &&
        typeof node.setValue === "function" &&
        !qhtmlVarBodyIsFunction(node)) {
      const resolved = iterationValueForExpression(node.value(), variableName, item);
      if (resolved.matched) {
        node.setValue(valueToSource(resolved.value));
      }
  ...
  class QHTMLForNode extends QHTMLTypedNode {
    ...
    setBodyJs(body) { this.setBody(body); }
        ...
        try {
	      ...
	      const trackGeneratedClone = (cloned, local) => {
	      ...
	      return values.map(item => {
	        ...
	        return this.children().map(child => {
	          ...
	          if (cloned instanceof QHTMLVar) {
	            local.setContextProperty(cloned.qhtmlName(), qhtmlVarValueForContext(cloned, local));
	            trackGeneratedClone(cloned, local);
	            return "";
	          }
	          if (cloned instanceof QHTMLScriptAction) {
	            qhtmlExecuteInlineScriptAction(cloned, local, this);
	            trackGeneratedClone(cloned, local);
	            return "";
	          }
  ...
  class QHTMLVar extends QHTMLTypedNode {
    constructor(name = "", attributes = {}, body = "") {
      super("q-var", name, attributes);
      this.setQHTMLType("QHTMLVar");
      this._value = attributes.value == null ? "" : String(attributes.value);
      this._body = trim(body);
    }
    value() { return this._value; }
    ...
    setBody(body) { this._body = trim(body); }
    setBodyJs(body) { this.setBody(body); }
    jsValue(contextNode = this.parent()) { return qhtmlVarValueForContext(this, contextNode); }
    renderHtml() { return ""; }
    sourceQHTML(indentLevel = 0) {
      if (this._body !== "") {
        return qhtmlSourceBlock("q-var " + this.qhtmlName(), this._body, indentLevel);
      }
      return qhtmlSourceIndent(indentLevel) + "q-var " + this.qhtmlName() + (this._value !== "" ? ": " + this._value : "");
    }
    toJsonObject() { return Object.assign(super.toJsonObject(), bodyJsonFields(this._body), { value: this._value }); }
  }
...
(function () {
  ...
  const qhtmlComponentScopeNodeCache = new WeakMap();
  const qhtmlReferenceNameMapCache = new WeakMap();
  ...
  function walkQHTMLNode(node, visitor, visited = new Set()) {
    ...
    if (typeof node.ownedReferenceMembers === "function") {
      node.ownedReferenceMembers().forEach((member) => {
        walkQHTMLNode(member, visitor, visited);
      });
    }
  }
  ...
  function applyCssShortcutAssignment(domElement, propertyNode, registry) {
    ...
    const applyStyleValue = function () {
      const nextGeometryValue = cssGeometryReferenceValue(cssName, rawValue, domElement, registry);
      const nextValue = typeof nextGeometryValue !== "undefined"
        ? nextGeometryValue
        : cssShortcutValue(rawValue, domElement, propertyNode, registry);
  ...
  function hasCssRuntimeUnit(value) {
    const parsed = parseCssRuntimeValue(value);
    return Number.isFinite(parsed.number) &&
      /^(?:%|px|em|rem|vw|vh|vmin|vmax|ch|ex|cm|mm|in|pt|pc|deg|rad|turn|s|ms)$/.test(parsed.unit);
  }
  ...
  function normalizeQHTMLPropertyRuntimeValue(domElement, propertyName, value, previousValue) {
    if (value instanceof QHTMLCssRuntimeValue) {
      return value;
    }
    if (typeof value === "number" && Number.isFinite(value)) {
      const previous = parseCssRuntimeValue(previousValue);
      if (/^(?:%|px|em|rem|vw|vh|vmin|vmax|ch|ex|cm|mm|in|pt|pc|deg|rad|turn|s|ms)$/.test(previous.unit)) {
        return new QHTMLCssRuntimeValue(domElement, propertyName, `${value}${previous.unit}`);
      }
      return value;
    }
    if (hasCssRuntimeUnit(value)) {
      return new QHTMLCssRuntimeValue(domElement, propertyName, String(value == null ? "" : value));
    }
    return value;
  }
  ...
  function qhtmlScriptObjectNodeFor(caller, scriptNode, registry) {
    if (scriptNode) {
      if (qhtmlNodeType(scriptNode) === "QHTMLEventHandler") {
        return typeof scriptNode.parent === "function" ? scriptNode.parent() : null;
      }
      return scriptNode;
    }
    return qhtmlNodeForReferenceTarget(caller, registry);
  }

  function qhtmlRuntimeValueForObjectNode(node, registry, caller) {
    if (!node) {
      return null;
    }
    const type = qhtmlNodeType(node);
    if (isQHTMLAnimationNodeType(type)) {
      const owner = ownerElementForQHTMLNode(node, registry) ||
        (caller && caller.ownerElement) ||
        (isDomElementLike(caller) ? caller : null) ||
        (registry && registry.rootElement);
      return runtimeAnimationForNode(node, registry, owner) || node;
    }
    if (type === "QHTMLComponentInstance" ||
      ...
      const value = runtimeValueForQHTMLReference(node, registry, caller);
      return typeof value === "undefined" ? node : value;
    }
    return node;
  }

  function qhtmlScriptObjectFor(caller, scriptNode, registry) {
    return qhtmlRuntimeValueForObjectNode(
      qhtmlScriptObjectNodeFor(caller, scriptNode, registry),
      registry,
      caller
    );
  }

  function qhtmlScriptObjectParentFor(caller, scriptNode, registry) {
    const objectNode = qhtmlScriptObjectNodeFor(caller, scriptNode, registry);
    const parentNode = objectNode && typeof objectNode.parent === "function" ? objectNode.parent() : null;
    return parentNode;
  }
  ...
  function executionContextFor(domElement, registry, parameterNames, scriptNode = null, referencedNames = null) {
    ...
    reserve("object", function object() {
      return qhtmlScriptObjectFor(domElement, scriptNode, sourceRegistry);
    });
    reserve("objectParent", function objectParent() {
      return qhtmlScriptObjectParentFor(domElement, scriptNode, sourceRegistry);
    });
  ...
  function teardownQHTML7Host(element) {
    ...
    if (registry) {
      ...
      if (registry.nodesByUuid) {
        registry.nodesByUuid.forEach((node) => {
          const object = node && node.__qhtmlRuntimeObject;
          if (object && typeof object.stop === "function") {
            object.stop();
          }
        });
      }
  ...
  function ownQHTMLRuntimeMember(owner, name, type) {
    ...
    if (Object.prototype.hasOwnProperty.call(descriptor, "value")) {
      ...
      if ((type === "QHTMLPropertyAnimation" ||
           type === "QHTMLAnimationGroup" ||
           type === "QHTMLSequentialAnimation" ||
           type === "QHTMLParallelAnimation") &&
          value &&
          value.qhtmlNode &&
          qhtmlNodeType(value.qhtmlNode) === type) {
        return value;
      }
  ...
  function runtimeValueForQHTMLReference(referenceNode, registry, selfElement) {
    ...
    if (type === "QHTMLContextPropertyPointer") {
      return typeof referenceNode.resolveTarget === "function"
        ? referenceNode.resolveTarget(registry.tree)
        : referenceNode;
    }
    ...
    if (type === "QHTMLPropertyAnimation" ||
        ...
        type === "QHTMLSequentialAnimation" ||
        type === "QHTMLParallelAnimation") {
      const owner = selfElement || ownerElementForQHTMLNode(referenceNode, registry) || registry.rootElement;
      const boundValue = ownQHTMLRuntimeMember(owner, name, type);
      if (typeof boundValue !== "undefined") {
        return boundValue;
      }
      return runtimeAnimationForNode(referenceNode, registry, owner);
    }
  ...
  function qhtmlReferenceRevisionForNode(node) {
    if (node && typeof node.referenceRevision === "function") {
      return node.referenceRevision();
    }
    const root = node && typeof node.rootNode === "function" ? node.rootNode() : node;
    return root && typeof root._qhtmlReferenceRevision === "number" ? root._qhtmlReferenceRevision : 0;
  }

  function qhtmlComponentScopeNodeForReferenceTarget(target, registry) {
    const node = qhtmlNodeForReferenceTarget(target, registry);
    if (node) {
      const revision = qhtmlReferenceRevisionForNode(node);
      const cached = qhtmlComponentScopeNodeCache.get(node);
      if (cached && cached.revision === revision) {
        return cached.scopeNode;
      }
      let current = node;
      while (current) {
        if (qhtmlNodeType(current) === "QHTMLComponentInstance" ||
            qhtmlNodeType(current) === "QHTMLDomTree") {
          qhtmlComponentScopeNodeCache.set(node, { revision, scopeNode: current });
          return current;
        }
        current = typeof current.parent === "function" ? current.parent() : null;
      }
      qhtmlComponentScopeNodeCache.set(node, { revision, scopeNode: node });
      return node;
    }
    let current = node || null;
    while (current) {
  ...
  function qhtmlReferenceNameMap(target, registry) {
    const node = qhtmlComponentScopeNodeForReferenceTarget(target, registry);
    if (!node) {
      return Object.create(null);
    }
    const revision = qhtmlReferenceRevisionForNode(node);
    const cached = qhtmlReferenceNameMapCache.get(node);
    if (cached && cached.revision === revision) {
      return Object.assign(Object.create(null), cached.map);
    }
    const out = Object.create(null);
    const seenUuids = new Set();

    const isComponentReferenceNode = function (sourceNode) {
      const type = qhtmlNodeType(sourceNode);
      return type === "QHTMLComponentInstance" ||
        type === "QHTMLComponentDefinition";
    };

    const isContextPropertyNode = function (sourceNode) {
      return qhtmlNodeType(sourceNode) === "QHTMLContextPropertyPointer";
    };

    const isNamedDomReferenceNode = function (sourceNode) {
      return qhtmlNodeType(sourceNode) === "QHTMLDomElement" &&
        typeof sourceNode.tagName === "function" &&
        qhtmlNodeName(sourceNode) !== sourceNode.tagName();
    };

    const isLocalReferenceBearingNode = function (sourceNode) {
      const type = qhtmlNodeType(sourceNode);
      return type === "QHTMLComponentInstance" ||
        type === "QHTMLComponentDefinition" ||
        isNamedDomReferenceNode(sourceNode) ||
        type === "QHTMLProperty" ||
        type === "QHTMLPropertyAssignment" ||
        type === "QHTMLFunction" ||
        type === "QHTMLSignal" ||
        type === "QHTMLEvent" ||
        type === "QHTMLContextPropertyPointer" ||
    ...
    const addReferenceNode = function (sourceNode, overwrite, predicate) {
      const accepts = predicate || isLocalReferenceBearingNode;
      const name = qhtmlNodeName(sourceNode);
      const uuid = qhtmlNodeUuid(sourceNode);
      if (name && uuid && accepts(sourceNode) &&
          !seenUuids.has(uuid) &&
    ...
    const addComponentReferenceNode = function (sourceNode, overwrite) {
      if (isComponentReferenceNode(sourceNode)) {
        addReferenceNode(sourceNode, overwrite, isComponentReferenceNode);
      }
    };

    const addContextPropertyNode = function (sourceNode, overwrite) {
      if (isContextPropertyNode(sourceNode)) {
        addReferenceNode(sourceNode, overwrite, isContextPropertyNode);
      }
    };

    const addDirectReferenceChildren = function (sourceNode, overwrite) {
      if (!sourceNode) {
        return;
      }
      ...
      for (let index = 0; index < count; index += 1) {
        ...
        addReferenceNode(child, overwrite);
        addSlotReferenceNodes(child, overwrite);
      }
      if (sourceNode === node && typeof sourceNode.ownedReferenceMembers === "function") {
        sourceNode.ownedReferenceMembers().forEach((member) => {
          addReferenceNode(member, overwrite);
          addSlotReferenceNodes(member, overwrite);
        });
      }
    };

    const addSlotReferenceNodes = function (sourceNode, overwrite, insideSlot = false) {
      if (!sourceNode) {
        return;
      }
      const sourceType = qhtmlNodeType(sourceNode);
      if (!insideSlot &&
          (sourceType === "QHTMLComponentDefinition" || sourceType === "QHTMLComponentInstance")) {
        return;
      }
      if (insideSlot) {
        addReferenceNode(sourceNode, overwrite);
      }
      const nextInsideSlot = insideSlot || sourceType === "QHTMLComponentSlot";
      const count = typeof sourceNode.childCount === "function" ? sourceNode.childCount() : 0;
      for (let index = 0; index < count; index += 1) {
        const child = sourceNode.childAt(index);
        ...
        if (nextInsideSlot &&
            child !== sourceNode &&
            (childType === "QHTMLComponentDefinition" || childType === "QHTMLComponentInstance")) {
          addReferenceNode(child, overwrite);
          continue;
        }
        addSlotReferenceNodes(child, overwrite, nextInsideSlot);
      }
      if (typeof sourceNode.ownedReferenceMembers === "function") {
        sourceNode.ownedReferenceMembers().forEach((member) => {
          const memberType = qhtmlNodeType(member);
          if (nextInsideSlot &&
              member !== sourceNode &&
              (memberType === "QHTMLComponentDefinition" || memberType === "QHTMLComponentInstance")) {
            addReferenceNode(member, overwrite);
            return;
          }
          addSlotReferenceNodes(member, overwrite, nextInsideSlot);
        });
      }
    };

    const addDirectContextProperties = function (sourceNode, overwrite) {
      const count = sourceNode && typeof sourceNode.childCount === "function" ? sourceNode.childCount() : 0;
      for (let index = 0; index < count; index += 1) {
        addContextPropertyNode(sourceNode.childAt(index), overwrite);
      }
      if (sourceNode === node && typeof sourceNode.ownedReferenceMembers === "function") {
        sourceNode.ownedReferenceMembers().forEach((member) => addContextPropertyNode(member, overwrite));
      }
    };

    const removeReferenceName = function (name) {
      if (name && Object.prototype.hasOwnProperty.call(out, String(name))) {
        delete out[String(name)];
      }
    };

    const shadowContextPropertiesWithLocalNames = function (sourceNode) {
      const count = sourceNode && typeof sourceNode.childCount === "function" ? sourceNode.childCount() : 0;
      for (let index = 0; index < count; index += 1) {
        const child = sourceNode.childAt(index);
        const name = qhtmlNodeName(child);
        if (name && isLocalReferenceBearingNode(child) && !isContextPropertyNode(child)) {
          removeReferenceName(name);
        }
      }
      if (sourceNode === node && typeof sourceNode.ownedReferenceMembers === "function") {
        sourceNode.ownedReferenceMembers().forEach((member) => {
          const name = qhtmlNodeName(member);
          if (name && isLocalReferenceBearingNode(member) && !isContextPropertyNode(member)) {
            removeReferenceName(name);
          }
        });
      }
    ...
    const addOneLevelComponentChildren = function (sourceNode, overwrite) {
      const count = sourceNode && typeof sourceNode.childCount === "function" ? sourceNode.childCount() : 0;
      for (let index = 0; index < count; index += 1) {
        const child = sourceNode.childAt(index);
        if (isComponentReferenceNode(child)) {
          addComponentReferenceNode(child, overwrite);
          continue;
        }
        const childCount = child && typeof child.childCount === "function" ? child.childCount() : 0;
        for (let childIndex = 0; childIndex < childCount; childIndex += 1) {
          const grandchild = child.childAt(childIndex);
          if (isComponentReferenceNode(grandchild)) {
            addComponentReferenceNode(grandchild, overwrite);
          }
        }
      }
      if (sourceNode === node && typeof sourceNode.ownedReferenceMembers === "function") {
        sourceNode.ownedReferenceMembers().forEach((member) => {
          if (isComponentReferenceNode(member)) {
            addComponentReferenceNode(member, overwrite);
          }
        });
      }
    };
    ...
    const addInheritedContextProperties = function () {
      ...
      for (let index = chain.length - 1; index >= 0; index -= 1) {
        const sourceNode = chain[index];
        addDirectContextProperties(sourceNode, true);
        shadowContextPropertiesWithLocalNames(sourceNode);
      }
    };

    const addAncestorComponentReferences = function () {
      const chain = [];
      let current = typeof node.parent === "function" ? node.parent() : null;
      while (current) {
        if (isComponentReferenceNode(current)) {
          chain.push(current);
        }
      ...
      for (let index = chain.length - 1; index >= 0; index -= 1) {
        addComponentReferenceNode(chain[index], false);
      }
    };

    const addSiblingComponentReferences = function () {
      let owner = typeof node.parent === "function" ? node.parent() : null;
      while (owner &&
          qhtmlNodeType(owner) !== "QHTMLComponentInstance" &&
          qhtmlNodeType(owner) !== "QHTMLDomTree") {
        owner = typeof owner.parent === "function" ? owner.parent() : null;
      }
      if (!owner) {
        return;
      }
      const count = typeof owner.childCount === "function" ? owner.childCount() : 0;
      for (let index = 0; index < count; index += 1) {
        const child = owner.childAt(index);
        if (child === node) {
          continue;
        }
        if (qhtmlNodeType(child) === "QHTMLComponentInstance") {
          addComponentReferenceNode(child, false);
          continue;
        }
        const childCount = child && typeof child.childCount === "function" ? child.childCount() : 0;
        for (let childIndex = 0; childIndex < childCount; childIndex += 1) {
          const grandchild = child.childAt(childIndex);
          if (grandchild !== node && qhtmlNodeType(grandchild) === "QHTMLComponentInstance") {
            addComponentReferenceNode(grandchild, false);
          }
        }
      }
    };

    addInheritedContextProperties();
    addAncestorComponentReferences();
    ...
    qhtmlReferenceNameMapCache.set(node, { revision, map: Object.assign(Object.create(null), out) });
    return out;
  }

  function qhtmlDirectReferenceNameMap(target, registry) {
    const sourceRegistry = registry || registryForQHTMLTarget(target);
    const node = qhtmlComponentScopeNodeForReferenceTarget(target, sourceRegistry);
    const out = Object.create(null);
    if (!node) {
      return out;
    }
  ...
  function resolveQHTMLReferenceNode(target, nameOrUUID, registry) {
    ...
    const map = qhtmlReferenceNameMap(target, sourceRegistry);
    const mappedUuid = map[key] || "";
    if (mappedUuid && sourceRegistry && sourceRegistry.nodesByUuid) {
      return sourceRegistry.nodesByUuid.get(mappedUuid) || null;
    }
    return undefined;
  }
  ...
  function installQHTMLReferenceAccess(domElement, registry, installAliases) {
    ...
    Array.from(aliasNames).forEach((name) => {
      ...
      if (currentDescriptor && !currentIsReferenceAlias && !currentIsRawQHTMLReference) {
        return;
      }
  ...
  function resolvePropertyValue(rawValue, domElement, propertyNode, registry) {
    ...
    if (text.indexOf("${") >= 0) {
      return evaluateQHTMLTextTemplate(stripMatchingQuotes(text), domElement, registry);
    }
  ...
  function propertyBindingExpression(rawValue) {
    const text = String(rawValue || "").replace(/\s*\/\*[\s\S]*?\*\/\s*$/, "").trim();
    if (!text || text.startsWith("`") || text.startsWith("{") || text.startsWith("[") || text.endsWith(")")) {
      return null;
    }
    if (typeof parsePrimitiveProperty(text) !== "undefined") {
      return null;
    }
    if (!/^(?:this|[A-Za-z_$][A-Za-z0-9_$]*)(?:\.[A-Za-z_$][A-Za-z0-9_$]*)+$/.test(text)) {
      return null;
    }
  ...
  function clearQHTMLPropertyBinding(entry) {
    if (!entry || !entry.binding) {
      return false;
    }
    const binding = entry.binding;
    if (binding.signal && typeof binding.signal.disconnect === "function") {
      binding.signal.disconnect(binding.update);
    }
    entry.binding = null;
    return true;
  }

  function queueQHTMLPropertyBinding(domElement, propertyName, propertyNode, rawValue, registry, sourceContextElement) {
    const binding = propertyBindingExpression(rawValue);
    if (!binding || !registry) {
      return false;
    }
    registry.propertyBindingRecords = registry.propertyBindingRecords || [];
    registry.propertyBindingRecords.push({
      domElement,
      propertyName,
      propertyNode,
      rawValue,
      ...
      sourceContextElement: sourceContextElement || domElement
    });
    const entry = domElement.__qhtmlProperties && domElement.__qhtmlProperties[propertyName];
    if (entry) {
      entry.bindingExpression = binding.expression;
    }
    return true;
  }

  function cancelQueuedQHTMLPropertyBinding(domElement, propertyName, registry) {
    if (!registry || !registry.propertyBindingRecords) {
      return false;
    }
    const previousLength = registry.propertyBindingRecords.length;
    registry.propertyBindingRecords = registry.propertyBindingRecords.filter((record) =>
      record.domElement !== domElement || record.propertyName !== propertyName
    );
    const entry = domElement.__qhtmlProperties && domElement.__qhtmlProperties[propertyName];
    if (entry && previousLength !== registry.propertyBindingRecords.length) {
      entry.bindingExpression = "";
    }
    return previousLength !== registry.propertyBindingRecords.length;
  }

  function installQHTMLPropertyBinding(record, registry) {
    const entry = record.domElement.__qhtmlProperties && record.domElement.__qhtmlProperties[record.propertyName];
    const source = resolvePath(record.sourcePath, registry, record.sourceContextElement || record.domElement);
    if (!entry) {
      throw new TypeError("Cannot bind QHTML property " + record.propertyName + ": destination property is not installed");
    }
    if (!source) {
      throw new ReferenceError("Cannot bind QHTML property " + record.propertyName + ": source " + record.sourcePath + " is undefined");
    }
    const signalName = `${record.sourceProperty}changed`;
    const signal = source[signalName] || source[`${record.sourceProperty}Changed`];
    if (typeof signal !== "function" || typeof signal.connect !== "function") {
      throw new TypeError("Cannot bind QHTML property " + record.propertyName + ": source signal " + signalName + " is not available");
    }
    clearQHTMLPropertyBinding(entry);
    const update = function (value) {
      entry.bindingApplying = true;
      try {
        record.domElement[record.propertyName] = value;
      } finally {
        entry.bindingApplying = false;
      }
    };
    entry.binding = {
    ...
    update(source[record.sourceProperty]);
    return true;
  }

  function flushQHTMLPropertyBindings(registry) {
    const records = registry && registry.propertyBindingRecords ? registry.propertyBindingRecords : [];
    registry.propertyBindingRecords = [];
    records.forEach((record) => installQHTMLPropertyBinding(record, registry));
  }

  const QHTML_INTERPOLATION_RESERVED_WORDS = new Set([
    "true", "false", "null", "undefined", "NaN", "Infinity",
    "return", "var", "let", "const", "if", "else", "for", "while",
    "do", "switch", "case", "break", "continue", "function", "new",
    "this", "event", "Math", "Number", "String", "Boolean", "Array",
    "Object", "JSON", "Date", "console", "window", "document"
  ]);

  function interpolationTemplateExpressions(source) {
    const expressions = [];
    String(source == null ? "" : source).replace(/\$\{([^}]+)\}/g, (match, expression) => {
  ...
  function stripInterpolationStringLiterals(source) {
    return String(source || "").replace(/(["'`])(?:\\.|(?!\1)[\s\S])*\1/g, " ");
  }

  function interpolationDependencyPaths(source) {
    const paths = new Set();
    interpolationTemplateExpressions(source).forEach((expression) => {
      const cleaned = stripInterpolationStringLiterals(expression)
        .replace(/\/\*[\s\S]*?\*\//g, " ")
        .replace(/\/\/[^\n\r]*/g, " ");
      const matcher = /\b(?:this\.)?[A-Za-z_$][A-Za-z0-9_$]*(?:\.[A-Za-z_$][A-Za-z0-9_$]*)*\b/g;
      let match = matcher.exec(cleaned);
      while (match) {
        const path = match[0];
        const first = path.split(".")[0];
        const nextChar = cleaned[matcher.lastIndex] || "";
        const startsWithThisPath = path.startsWith("this.");
        if (!QHTML_INTERPOLATION_RESERVED_WORDS.has(path) &&
            (startsWithThisPath || !QHTML_INTERPOLATION_RESERVED_WORDS.has(first)) &&
            nextChar !== "(") {
          paths.add(path);
        }
  ...
  function interpolationSignalFor(owner, propertyName) {
    if (!owner || !propertyName) {
      return null;
    }
    const lowerSignalName = `${propertyName}changed`;
    const camelSignalName = `${propertyName}Changed`;
    const signal = owner[lowerSignalName] || owner[camelSignalName];
    return typeof signal === "function" && typeof signal.connect === "function" ? signal : null;
  }

  function resolveInterpolationDependency(path, domElement, registry) {
    const parts = String(path || "").trim().split(".").filter(Boolean);
    if (parts.length === 0 || !registry) {
      return null;
    }
    ...
    let owner = null;
    if (parts.length === 0) {
      const scriptThis = qhtmlScriptThisFor(domElement, registry);
      owner = scriptThis && interpolationSignalFor(scriptThis, propertyName) ? scriptThis : domElement;
    } else if (parts.length === 1 && parts[0] === "this") {
      owner = qhtmlScriptThisFor(domElement, registry);
    } else {
      owner = resolvePath(parts.join("."), registry, domElement);
    }
    const signal = interpolationSignalFor(owner, propertyName);
    if (!signal) {
      return null;
    }
    return { owner, propertyName, signal };
  }

  function queueQHTMLInterpolationBinding(registry, domElement, source, apply, label) {
    if (!registry || !domElement || typeof apply !== "function" || String(source || "").indexOf("${") < 0) {
      return false;
    }
    const bindingKey = `${label || "interpolation"}:${String(source || "")}`;
    domElement.__qhtmlInterpolationBindingKeys = domElement.__qhtmlInterpolationBindingKeys || new Set();
    if (domElement.__qhtmlInterpolationBindingKeys.has(bindingKey)) {
      return false;
    }
    domElement.__qhtmlInterpolationBindingKeys.add(bindingKey);
    const record = {
      domElement,
    ...
    if (registry.interpolationBindingsReady === true) {
      return installQHTMLInterpolationBinding(record, registry);
    }
    registry.interpolationBindingRecords = registry.interpolationBindingRecords || [];
    registry.interpolationBindingRecords.push(record);
    return true;
  }
  ...
  function installQHTMLInterpolationBinding(record, registry) {
    ...
    const uniqueDependencies = dependencies.filter((dependency) => {
      const ownerUuid = dependency.owner && typeof dependency.owner.getAttribute === "function"
        ? dependency.owner.getAttribute("component-instance") || dependency.owner.getAttribute("qhtml-node") || ""
        : "";
      const key = `${dependency.propertyName}:${dependency.signal.__qhtmlSignalName || ""}:${ownerUuid}`;
      if (seen.has(key)) {
        return false;
      }
      seen.add(key);
    ...
    uniqueDependencies.forEach((dependency) => dependency.signal.connect(update));
    record.domElement.__qhtmlInterpolationBindings = record.domElement.__qhtmlInterpolationBindings || [];
    record.domElement.__qhtmlInterpolationBindings.push({
      label: record.label,
  ...
  function flushQHTMLInterpolationBindings(registry) {
    const records = registry && registry.interpolationBindingRecords ? registry.interpolationBindingRecords : [];
    registry.interpolationBindingRecords = [];
    records.forEach((record) => installQHTMLInterpolationBinding(record, registry));
    if (registry) {
      registry.interpolationBindingsReady = true;
    }
  }
  ...
  function behaviorNodeForProperty(domElement, propertyName) {
    ...
    const children = qhtmlRuntimeDeclarationChildren(ownerNode);
  ...
  function behaviorAnimationNodes(behaviorNode) {
    const children = [];
  ...
  function behaviorAnimationNode(behaviorNode) {
    return behaviorAnimationNodes(behaviorNode)[0] || null;
  }

  function runtimeBehaviorAnimationForNode(behaviorNode, registry, ownerElement) {
    const children = behaviorAnimationNodes(behaviorNode);
    if (children.length === 0) {
      return null;
    }
    if (children.length === 1) {
      return runtimeAnimationForNode(children[0], registry, ownerElement);
    }
    return createLiveAnimationGroup(behaviorNode, ownerElement, registry, "sequential");
  }

  function behaviorPropertyPath(behaviorNode) {
    return stripMatchingQuotes(
      String(behaviorNode && typeof behaviorNode.propertyName === "function"
        ? behaviorNode.propertyName()
        : qhtmlNodeName(behaviorNode) || "")
    ).trim();
  }

  function resolveBehaviorTarget(behaviorNode, ownerElement, registry) {
    const path = behaviorPropertyPath(behaviorNode);
    if (!path) {
      return { target: null, property: "" };
    }
    const targetMatch = path.match(/^([\s\S]+)\.([A-Za-z_$][A-Za-z0-9_$-]*)$/);
    if (targetMatch) {
      return {
        target: evaluateQHTMLValueExpression(targetMatch[1], ownerElement, registry),
        property: targetMatch[2]
      };
    }
    return {
      target: ownerElement,
  ...
  function ensureBehaviorTargetProperty(target, propertyName, behaviorNode, registry) {
    if (!target || !propertyName) {
      return;
    }
    if (target.__qhtmlProperties && Object.prototype.hasOwnProperty.call(target.__qhtmlProperties, propertyName)) {
      return;
    }
    bindBehaviorTargetProperty(target, behaviorNode, registry, propertyName);
  }

  function registerBehaviorDeclaration(ownerElement, behaviorNode, registry) {
    const resolved = resolveBehaviorTarget(behaviorNode, ownerElement, registry);
    if (!resolved.target || !resolved.property) {
      return false;
    }
    ensureBehaviorTargetProperty(resolved.target, resolved.property, behaviorNode, registry);
    resolved.target.__qhtmlBehaviorStates = resolved.target.__qhtmlBehaviorStates || Object.create(null);
    const state = resolved.target.__qhtmlBehaviorStates[resolved.property] || {};
    state.behaviorNode = behaviorNode;
    state.ownerElement = ownerElement;
    state.target = resolved.target;
    state.propertyName = resolved.property;
    resolved.target.__qhtmlBehaviorStates[resolved.property] = state;
    return true;
  }

  function bindBehaviorDeclarations(ownerElement, qhtmlNode, registry) {
    const children = qhtmlRuntimeDeclarationChildren(qhtmlNode);
    for (let index = 0; index < children.length; index += 1) {
      const child = children[index];
      if (qhtmlNodeType(child) === "QHTMLBehavior") {
        registerBehaviorDeclaration(ownerElement, child, registry);
      }
    }
  }

  function configureBehaviorAnimation(animation, target, propertyName, fromValue, toValue, state) {
    ...
    const type = qhtmlNodeType(node);
    if (type === "QHTMLPropertyAnimation") {
      animation.__qhtmlBehaviorState = state || null;
      animation.__qhtmlBehaviorOverride = {
        target,
    ...
    if (typeof animation.children === "function") {
      animation.children().forEach((childAnimation) => {
        configureBehaviorAnimation(childAnimation, target, propertyName, fromValue, toValue, state);
      });
    }
  }
  ...
  function startPropertyBehavior(domElement, propertyNode, registry, propertyName, nextValue, previousValue, transactionId) {
    ...
    const behaviorNode = state.behaviorNode || behaviorNodeForProperty(domElement, propertyName);
    const animation = runtimeBehaviorAnimationForNode(behaviorNode, registry, state.ownerElement || domElement);
    ...
    state.target = domElement;
    state.propertyName = propertyName;
    state.behaviorNode = behaviorNode;
    state.ownerElement = state.ownerElement || domElement;
    ...
    configureBehaviorAnimation(animation, domElement, propertyName, previousValue, nextValue, state);
  ...
  function bindProperty(domElement, propertyNode, registry) {
    try {
      ...
      const rawValue = typeof propertyNode.value === "function" ? propertyNode.value() : "";
      const resolvedValue = normalizeQHTMLPropertyRuntimeValue(
        domElement,
        propertyName,
        resolvePropertyValue(rawValue, domElement, propertyNode, registry),
        undefined
      );
      ...
      Object.defineProperty(domElement, propertyName, {
        ...
        set(nextValue) {
          ...
          if (behaviorState && behaviorState.suppress) {
            entry.value = normalizedValue;
            dispatchPropertyChange(domElement, propertyNode, propertyName, normalizedValue, transactionId);
            return;
          }
          ...
          if (startPropertyBehavior(domElement, propertyNode, registry, propertyName, normalizedValue, entry.value, transactionId) ||
              startPropertyTransition(domElement, propertyNode, registry, propertyName, normalizedValue, entry.value, transactionId)) {
            return;
          }
          entry.value = normalizedValue;
          dispatchPropertyChange(domElement, propertyNode, propertyName, normalizedValue, transactionId);
      ...
      queueQHTMLPropertyBinding(domElement, propertyName, propertyNode, rawValue, registry, domElement);
    } catch (error) {
  ...
  function bindBehaviorTargetProperty(domElement, behaviorNode, registry, resolvedPropertyName = "") {
    if (!domElement || !behaviorNode) {
      return;
    }
    const propertyName = resolvedPropertyName ||
      (typeof behaviorNode.propertyName === "function" ? behaviorNode.propertyName() : qhtmlNodeName(behaviorNode));
  ...
  function bindBehaviorTargetProperties(domElement, qhtmlNode, registry) {
    ...
    for (let index = 0; index < children.length; index += 1) {
      const child = children[index];
      if (qhtmlNodeType(child) === "QHTMLBehavior") {
        const propertyName = behaviorPropertyPath(child);
        if (propertyName && propertyName.indexOf(".") < 0) {
          bindBehaviorTargetProperty(domElement, child, registry, propertyName);
        }
      }
    }
  }
  ...
  function bindComponentInstancePropertyAssignments(domElement, instanceNode, registry) {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      cancelQueuedQHTMLPropertyBinding(domElement, propertyName, registry);
      queueQHTMLPropertyBinding(domElement, propertyName, child, rawValue, registry, assignmentContext);
    }
  }

  function propertyNodesForDefinition(definitionNode) {
    const properties = [];
    const roots = QHTMLTypes && typeof QHTMLTypes.qhtmlEffectiveComponentDefinitionChildren === "function"
      ? QHTMLTypes.qhtmlEffectiveComponentDefinitionChildren(definitionNode)
      : (definitionNode ? [definitionNode] : []);
    roots.forEach((root) => walkQHTMLNode(root, (node) => {
  ...
  function createComponentDefinitionProxy(definitionNode, registry) {
    ...
    const definitionProxy = {
      ...
      propertySignals: {},
      create(parentNode, properties = {}) {
        return definitionNode.create(parentNode, properties);
      },
      remove() {
        return definitionNode.remove();
      }
    };
  ...
  function removeRuntimeQHTMLNode(runtimeObject, qhtmlNode) {
    if (runtimeObject && typeof runtimeObject.stop === "function") {
      runtimeObject.stop();
    }
    return qhtmlNode && typeof qhtmlNode.remove === "function" ? qhtmlNode.remove() : false;
  }
  ...
  function createLiveTimer(timerNode, ownerElement, registry) {
    ...
    liveTimer.remove = function () {
      return removeRuntimeQHTMLNode(liveTimer, timerNode);
    };
  ...
  function animationRawAssignment(animationNode, name) {
    const lowerName = String(name || "").toLowerCase();
    const count = typeof animationNode.childCount === "function" ? animationNode.childCount() : 0;
    for (let index = 0; index < count; index += 1) {
      const child = animationNode.childAt(index);
      if (qhtmlNodeType(child) === "QHTMLPropertyAssignment" && qhtmlNodeName(child).toLowerCase() === lowerName) {
        return typeof child.value === "function" ? child.value() : "";
      }
    }
    return "";
  }

  function explicitAnimationProperty(animationNode, ownerElement, registry, fallback) {
    return animationAssignment(animationNode, "property", ownerElement, registry,
      animationAssignment(animationNode, "propertyName", ownerElement, registry,
        animationAssignment(animationNode, "targetProperty", ownerElement, registry, fallback)));
  }

  function resolveAnimationTargetReference(animationNode, ownerElement, registry, fallbackProperty) {
    const rawTarget = stripMatchingQuotes(animationRawAssignment(animationNode, "target")).trim();
    const hasExplicitProperty = animationHasAssignment(animationNode, "property") ||
      animationHasAssignment(animationNode, "propertyName") ||
      animationHasAssignment(animationNode, "targetProperty");
    if (rawTarget && !hasExplicitProperty) {
      const targetMatch = rawTarget.match(/^([\s\S]+)\.([A-Za-z_$][A-Za-z0-9_$]*)$/);
      if (targetMatch) {
        return {
          target: evaluateQHTMLValueExpression(targetMatch[1], ownerElement, registry),
          property: targetMatch[2]
    ...
    return {
      target: animationAssignment(animationNode, "target", ownerElement, registry, ownerElement),
      property: explicitAnimationProperty(animationNode, ownerElement, registry, fallbackProperty)
    };
  }
  ...
  function createLivePropertyAnimation(animationNode, ownerElement, registry) {
    ...
    animationNode.__qhtmlRuntimeObject = animationObject;
    ...
    animationObject.__qhtmlApplyFrame = function (value) {
      const blockTargetSignals = animationObject.target &&
        ((animationObject.target.style && isCssShortcutAssignmentName(animationObject.property)) ||
          !timerBool(animationObject.emitInterpolatedValues, false));
      const applyFrame = function () {
        const behaviorState = animationObject.__qhtmlBehaviorState || null;
        const previousSuppress = behaviorState ? behaviorState.suppress : false;
        if (behaviorState) {
          behaviorState.suppress = true;
        }
        try {
        writeAnimationFrameProperty(
          ...
          if (behaviorState) {
            behaviorState.suppress = previousSuppress;
          }
        }
      };
      if (blockTargetSignals) {
        withQHTMLSignalsBlocked(animationObject.target, applyFrame);
      } else {
        applyFrame();
      }
    };

    animationObject.__qhtmlEmitSuppressedFinalValue = function () {
      if (timerBool(animationObject.emitInterpolatedValues, false) ||
          !animationObject.target ||
          !animationObject.property ||
          (animationObject.target.style && isCssShortcutAssignmentName(animationObject.property))) {
        return;
      }
      const entry = animationObject.target.__qhtmlProperties &&
        animationObject.target.__qhtmlProperties[animationObject.property];
      if (!entry) {
        return;
      }
      dispatchPropertyChange(
        animationObject.target,
        entry.qhtmlNode || animationNode,
        animationObject.property,
        entry.value,
        currentPropertyTransactionId()
      );
    ...
    animationObject.refresh = function () {
      const resolvedTarget = resolveAnimationTargetReference(
        animationNode,
        ownerElement,
        registry,
        inferAnimationPropertyName(animationName)
      );
      animationObject.target = resolvedTarget.target;
      animationObject.property = resolvedTarget.property;
      ...
      const behaviorOverride = animationObject.__qhtmlBehaviorOverride || null;
      if (behaviorOverride) {
        const hasExplicitTarget = animationHasAssignment(animationNode, "target");
        const hasExplicitProperty = animationHasAssignment(animationNode, "property") ||
          animationHasAssignment(animationNode, "propertyName") ||
          animationHasAssignment(animationNode, "targetProperty");
        if (!hasExplicitTarget && !hasExplicitProperty) {
          animationObject.target = behaviorOverride.target;
          animationObject.property = behaviorOverride.property;
        }
        if (!animationHasAssignment(animationNode, "from") &&
            !animationHasAssignment(animationNode, "start") &&
            !animationHasAssignment(animationNode, "startValue")) {
          animationObject.from = behaviorOverride.from;
        }
        if (!animationHasAssignment(animationNode, "to") &&
            !animationHasAssignment(animationNode, "end") &&
            !animationHasAssignment(animationNode, "endValue")) {
          animationObject.to = behaviorOverride.to;
        }
      }
    ...
    animationObject.step = function () {
      try {
        ...
        const finish = function () {
          ...
          animationObject.__qhtmlEmitSuppressedFinalValue();
    ...
    animationObject.remove = function () {
      return removeRuntimeQHTMLNode(animationObject, animationNode);
    };
  ...
  function createLiveScriptAction(actionNode, ownerElement, registry) {
    ...
    actionNode.__qhtmlRuntimeObject = actionObject;
    ...
    actionObject.remove = function () {
      return removeRuntimeQHTMLNode(actionObject, actionNode);
    };
  ...
  function runtimeAnimationForNode(node, registry, ownerElement = null) {
    if (!node) {
      return null;
    }
    if (node.__qhtmlRuntimeObject) {
      return node.__qhtmlRuntimeObject;
    }
    const owner = ownerElement || ownerElementForQHTMLNode(node, registry) || (registry && registry.rootElement);
    const type = qhtmlNodeType(node);
    if (type === "QHTMLPropertyAnimation") {
      node.__qhtmlRuntimeObject = createLivePropertyAnimation(node, owner, registry);
    } else if (type === "QHTMLScriptAction") {
      node.__qhtmlRuntimeObject = createLiveScriptAction(node, owner, registry);
    } else if (type === "QHTMLSequentialAnimation" || type === "QHTMLParallelAnimation") {
      animationGroupChildNodes(node).forEach((child) => runtimeAnimationForNode(child, registry, owner));
      node.__qhtmlRuntimeObject = createLiveAnimationGroup(
        node,
        owner,
        registry,
        type === "QHTMLParallelAnimation" ? "parallel" : "sequential"
      );
    }
    return node.__qhtmlRuntimeObject || null;
  }
  ...
  function createLiveAnimationGroup(groupNode, ownerElement, registry, mode) {
    ...
    groupNode.__qhtmlRuntimeObject = groupObject;
    ...
    groupObject.children = function () {
      return animationGroupChildNodes(groupNode).map((child) => runtimeAnimationForNode(child, registry, ownerElement)).filter(Boolean);
    };
    ...
    groupObject.remove = function () {
      return removeRuntimeQHTMLNode(groupObject, groupNode);
    };
    groupObject.start = function () {
      ...
      if (mode === "parallel") {
        ...
        const runNext = function (index) {
          ...
          groupObject.__qhtmlActiveChildren = [child];
          let settled = false;
          const finishChild = function () {
            if (settled) {
              return;
            }
            settled = true;
            if (child.finished && typeof child.finished.disconnect === "function") {
              child.finished.disconnect(finishChild);
            }
            runNext(index + 1);
          };
          if (child.finished && typeof child.finished.connect === "function") {
            child.finished.connect(finishChild);
          }
          child.start();
          if (!child.running) {
            finishChild();
  ...
  function applyQHTMLStyle(domElement, styleDef, options) {
    ...
    const applyStyleText = function () {
      const nextRawCssText = typeof styleDef.cssText === "function" ? styleDef.cssText() : "";
      const nextCssText = cssTextWithoutTransitionBlocks(cssTextWithoutStylePainters(nextRawCssText), registry);
      cssDeclarations(nextCssText).forEach((decl) => {
        const cssName = cssShortcutPropertyName(decl.name);
        if (defaultOnly && cssShortcutRawValue(domElement, cssName)) {
          return;
        }
        const cssValue = evaluateQHTMLTextTemplate(decl.value, domElement, registry);
        domElement.style.setProperty(cssName, cssValue);
      });
    ...
    queueQHTMLInterpolationBinding(registry, domElement, rawCssText, applyStyleText, `q-style:${styleName || qhtmlNodeUuid(styleDef.node || styleDef) || "anonymous"}`);
    reapplyPaintTargetsForElement(domElement, registry);
  }
  ...
  function applyInlineQHTMLStyle(element, cssText, options) {
    ...
    const applyInlineStyleText = function () {
      cssDeclarations(cssTextWithoutStylePainters(cssText)).forEach((decl) => {
        const cssName = cssShortcutPropertyName(decl.name);
        if (defaultOnly && cssShortcutRawValue(element, cssName)) {
          return;
        }
        const cssValue = evaluateQHTMLTextTemplate(decl.value, element, registry);
    ...
    queueQHTMLInterpolationBinding(registry, element, cssText, applyInlineStyleText, "q-theme:inline-style");
    if (registry) {
      reapplyPaintTargetsForElement(element, registry);
    }
  }
  ...
  function isQHTMLAnimationNodeType(type) {
    return type === "QHTMLPropertyAnimation" ||
      type === "QHTMLSequentialAnimation" ||
      type === "QHTMLParallelAnimation" ||
      type === "QHTMLScriptAction";
  }

  function bindAnimationDeclaration(domElement, animationNode, registry) {
    const animation = runtimeAnimationForNode(animationNode, registry, domElement);
    const name = qhtmlNodeName(animationNode);
    if (name) {
      domElement[name] = animation;
    }
    return animation;
  }

  function qhtmlRuntimeDeclarationChildren(qhtmlNode) {
    const out = [];
    const seen = new Set();
    const add = function (node) {
      ...
      const uuid = qhtmlNodeUuid(node);
      const key = uuid || String(out.length);
      if (seen.has(key)) {
        return;
      }
      seen.add(key);
    ...
    const count = qhtmlNode && typeof qhtmlNode.childCount === "function" ? qhtmlNode.childCount() : 0;
    for (let index = 0; index < count; index += 1) {
      add(qhtmlNode.childAt(index));
    }
    if (qhtmlNode && typeof qhtmlNode.ownedReferenceMembers === "function") {
      qhtmlNode.ownedReferenceMembers().forEach(add);
    }
    return out;
  }
  ...
  function applyLegacyUnknownFragment(domElement, source, registry) {
    ...
    if (legacyPropertyScriptMatch) {
      ...
      const value = normalizeQHTMLPropertyRuntimeValue(
        domElement,
        propertyName,
        legacyScriptBlockValue(domElement, legacyPropertyScriptMatch[2], registry),
        undefined
      );
      domElement.__qhtmlProperties = Object.assign(domElement.__qhtmlProperties || {}, {
      ...
      try {
        Object.defineProperty(domElement, propertyName, {
          ...
          set(nextValue) {
            const entry = domElement.__qhtmlProperties[propertyName];
            entry.value = normalizeQHTMLPropertyRuntimeValue(domElement, propertyName, nextValue, entry.value);
          }
        });
      } catch (error) {
    ...
    while (index < text.length) {
      ...
      if (blockMatch) {
        ...
        if (name === "text") {
          ...
          const applyText = function () {
            domElement.textContent = evaluateQHTMLTextTemplate(template, domElement, registry);
          };
          applyText();
          queueQHTMLInterpolationBinding(registry, domElement, template, applyText, "legacy-text");
        } else if (name === "html") {
          const template = body.trim();
          const applyHtml = function () {
            domElement.innerHTML = evaluateQHTMLTextTemplate(template, domElement, registry);
          };
          applyHtml();
          queueQHTMLInterpolationBinding(registry, domElement, template, applyHtml, "legacy-html");
        }
        applied = true;
      ...
      if (text.slice(index).startsWith("q-script")) {
        ...
        valueSource = scriptOpen >= 0 && scriptClose >= 0 ? text.slice(index, scriptClose + 1) : "";
        ...
        valueSource = quoted.value;
        value = resolvePropertyValue(quoted.value, domElement, null, registry);
        index = quoted.end;
      } else {
        const bare = readLegacyBareValue(text, index);
        valueSource = bare.value;
        value = resolvePropertyValue(bare.value, domElement, null, registry);
        index = bare.end;
      }
      domElement.setAttribute(name, String(value == null ? "" : value));
      if (valueSource.indexOf("${") >= 0) {
        const assignmentSource = valueSource;
        const applyAttribute = function () {
          const resolved = resolvePropertyValue(assignmentSource, domElement, null, registry);
          domElement.setAttribute(name, String(resolved == null ? "" : resolved));
        };
        queueQHTMLInterpolationBinding(registry, domElement, assignmentSource, applyAttribute, `legacy-attribute:${name}`);
      }
      applied = true;
    }
    return applied;
  ...
  function applyRuntimeTextFragments(domElement, qhtmlNode, registry, ignoreUnknownFragments) {
    ...
    queueQHTMLInterpolationBinding(
      registry,
      domElement,
      fragments.map((fragment) => qhtmlNodeBodyText(fragment)).join("\n"),
      applyFragments,
      "text-fragments"
    );
  }
  ...
  function bindComponentFacade(domElement, registry) {
    ...
    domElement.qhtmlName = function qhtmlName() {
      return qhtmlNodeName(domElement.qhtmlNode);
    };
    domElement.qhtmlUUID = function qhtmlUUID() {
      return qhtmlNodeUuid(domElement.qhtmlNode);
    };
    domElement.qhtmlType = function qhtmlType() {
      return qhtmlNodeType(domElement.qhtmlNode);
    };
    Object.defineProperty(domElement, "remove", {
      configurable: true,
      enumerable: false,
      writable: true,
      value() {
        const node = domElement.qhtmlNode;
  ...
  function bindRuntimeChildren(domElement, qhtmlNode, registry) {
    ...
    const runtimeChildren = qhtmlRuntimeDeclarationChildren(qhtmlNode);
    for (const child of runtimeChildren) {
      ...
      } else if (isQHTMLAnimationNodeType(childType)) {
        bindAnimationDeclaration(domElement, child, registry);
    ...
    bindBehaviorDeclarations(domElement, qhtmlNode, registry);
  ...
  function bindComponentDomRuntime(rootElement, tree) {
    ...
    const registry = {
      ...
      interpolationBindingRecords: [],
      interpolationBindingsReady: false,
      rootElement,
      tree,
      globals: globalScope
    };
    ...
    flushQHTMLPropertyBindings(registry);
    flushQHTMLInterpolationBindings(registry);
